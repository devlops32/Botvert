...
# ==================== ОБРАБОТЧИК ОПЛАТЫ (ИСПРАВЛЕННЫЙ - ТОВАР НЕ СКРЫВАЕТСЯ) ====================
...
async def user_paid(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Создаем заказ (НО НЕ МЕНЯЕМ СТАТУС ТОВАРА!)
    ...
    if order_id:
        # ❌ НЕ УДАЛЯЕМ ТОВАР С ВИТРИНЫ!
        # ТОВАР ОСТАЕТСЯ ДОСТУПНЫМ ПОКА АДМИН НЕ ПОДТВЕРДИТ ОПЛАТУ
        # db.update_product_availability(product_id, False) ← ЭТУ СТРОКУ УБРАЛИ!
        ...
        # Отправляем уведомление админам для подтверждения
...
# ==================== ОБРАБОТЧИКИ ПОДТВЕРЖДЕНИЯ ОПЛАТЫ (АДМИН) ====================
...
async def admin_confirm_payment(callback: types.CallbackQuery, state: FSMContext):
    """Админ подтверждает оплату - ТОЛЬКО ТУТ УДАЛЯЕМ ТОВАР С ВИТРИНЫ"""
    ...
    # Получаем product_id из заказа
    product_id = order['product_id']
    
    # Подтверждаем оплату
    if db.confirm_payment(order_id):
        # ✅ ТОЛЬКО ТЕПЕРЬ УДАЛЯЕМ ТОВАР С ВИТРИНЫ!
        ...
        # Запускаем таймер на 30 минут для отправки фото
...
async def admin_reject_payment(callback: types.CallbackQuery, state: FSMContext):
    """Админ отклоняет оплату - ТОВАР ОСТАЕТСЯ НА ВИТРИНЕ"""
    if not db.is_admin(callback.from_user.id):
        await callback.answer("❌ У вас нет прав", show_alert=True)
        return
    ...
    # Отклоняем оплату (ТОВАР НЕ ТРОГАЕМ, ОСТАЕТСЯ НА ВИТРИНЕ)
...
async def send_photo_after_delay(bot, order_id: int):
    """Отправка фото через 30 минут после подтверждения оплаты"""
    ...
    # Проверяем, отправлено ли уже фото
    if order['photo_sent']:
        return
    
    # Отправляем уведомление админу, что нужно отправить фото
    for admin_id in ADMIN_IDS:
        try:
