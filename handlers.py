...
async def cmd_start(message: types.Message, state: FSMContext):
    """Обработчик команды /start с полным логированием пользователя"""
    ...
    # Сохраняем пользователя с полной информацией
    db.add_user(
...
async def admin_send_photo_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not order:
        ...
        await callback.message.edit_text(
            f"❌ Заказ #{order_id} не найден в базе данных.\n\n"
            f"💡 Проверьте, что заказ был создан корректно."
        )
        return
...
async def admin_send_photo(message: types.Message, state: FSMContext):
    """Отправка фото покупателю с обработкой ошибок"""
    ...
    order = db.get_order_by_id(order_id)
    if not order:
        await message.answer(f"❌ Заказ #{order_id} не найден в базе данных")
        await state.clear()
        return
    ...
    status_msg = await message.answer(f"⏳ Отправка фото пользователю {user_id}...")
    ...
    try:
        ...
        await message.bot.send_photo(
            chat_id=user_id,
            photo=photo_file_id,
            caption=f"📸 Ваш товар: {order['product_name']}"
        )
        ...
        if "chat not found" in error_msg:
            error_text += (
                ...
                f"💡 Решение:\n"
                f"1. Попросите пользователя написать боту /start\n"
                f"2. После этого повторите отправку"
            )
        elif "bot was blocked" in error_msg:
            error_text += (
                f"Пользователь (ID: {user_id}) заблокировал бота.\n\n"
                f"💡 Решение:\n"
                f"1. Свяжитесь с пользователем другим способом\n"
                f"2. Попросите разблокировать бота"
            )
        elif "user is deactivated" in error_msg:
            error_text += (
                f"Аккаунт пользователя (ID: {user_id}) деактивирован.\n\n"
                f"💡 Решение:\n"
                f"Свяжитесь с пользователем другим способом"
            )
        else:
...
async def admin_select_description(callback: types.CallbackQuery, state: FSMContext):
    ...
    if db.add_product(city_id, product_name, quantity, price, description):
        ...
        # Отправляем сообщение админу с жирным шрифтом
        ...
        # Отправляем уведомление всем пользователям с жирным шрифтом
...
async def notify_users_about_new_product(bot, city_name: str, product_name: str, quantity: str, price: str, description: str):
    """Отправка уведомления о новом товаре всем пользователям с жирным шрифтом"""
...
"""
    ...
    for user in users[:10]:  # Показываем последних 10
        username = f"@{user['username']}" if user['username'] else "Нет username"
        text += f"\n• {user['user_id']} | {username} | {user['full_name']}"
        if user['is_admin']:
            text += " 👑"
